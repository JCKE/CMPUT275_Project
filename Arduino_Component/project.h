...
const int tile_gather_time = 200; // Time to mine from a tile in millis
...
UTFT myGLCD4(ITDB32S,10,11,12,13);
...
player *current_player = &green;
unit *current_unit = &green.units[0];
...
void update_unit_info(unit *sel_unit = current_unit, player *sel_player = current_player);
...
void update_assets(char asset_type, int asset_amount, char cursor_or_player, player *sel_player = current_player);
